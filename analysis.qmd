...
format:
  html:
    include-in-header:
      - text: "<style>figcaption { text-align: center; }</style>"
---
...
This is loaded into R as `raw_weights`. A much smaller table, `lives`, contains the list of rats, their (estimated) dates of birth, and dates of death. Rats only live for 2-3 years!
...
::: {#fig-replicated-plots layout-ncol=2}

```{r}
#| echo: false
#| label: fig-replicated-by-date
#| fig-cap: "Using date as the $x$ axis"

rats_colours <- scale_color_manual(values = c("#f6b26b", "#93c47d", "#e06666", "#8e7cc3", "#6d9eeb"))

ggplot(weights, aes(x = Date, y = Weight)) + 
  geom_line(aes(color = Rat), size = .8) +
  theme_minimal() +
...
```{r}
#| echo: false
#| label: fig-replicated-by-age
#| fig-cap: "Using age as the $x$ axis"
...
The final line graphs of the weight time series, recreated using R with Tidyverse.
:::
...
Plotting multiple time series using spreadsheet software is possible, but doing it in a robust way that allows visualising both by date and by series age is difficult. R on the other hand could do this without breaking a sweat - just check out [this notebook's source code](https://github.com/Foggalong/Rats-Stats/blob/main/analysis.qmd)!
